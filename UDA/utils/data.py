...
class Data:
    def __init__(self):
        ...
        self.pretrain = 'None' #'None'/'bert-base-cased'/'ELMo'...
        self.task = 'NER' # NER

        self.label_alphabet = Alphabet('label', True)
        self.entity_alphabet = Alphabet('label', True)
        self.entity_type = []
        self.position_type = []
        self.entity_dict = {}
    ...
    def show_data_summary(self):
        ...
        print("     LMRaw instance number: %s"%(len(self.raw_data_texts)))
    ...
    def build_entity_dict(self, input_direct):
        file_entity = {'person':'PER', 'location':'LOC', 'organization':'ORG', 'misc':'MISC'}
        file_list = os.listdir(input_direct)
        for idx, file in enumerate(file_list):
            file_name = file[:-4]
            entity_type = file_entity[file_name]
            entities = open(os.path.join(input_direct, file), 'r', encoding='utf-8').readlines()
    ...
    def build_alphabet_raw(self, input_file):
    ...
    def generate_instance(self, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance(self.train_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.entity_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
            self.raw_data_texts, self.raw_data_Ids = read_raw_instance(self.raw_data_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.entity_alphabet, self.entity_dict, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_instance(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.entity_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
        elif name == "test":
            self.test_texts, self.test_Ids = read_instance(self.test_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.entity_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = read_instance(self.raw_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.entity_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token)
        else:
    ...
    def read_config(self,config_file):
        ...
        the_item = 'raw_data_dir'
        if the_item in config:
            self.raw_data_dir = config[the_item]
        the_item = 'decode_dir'
        if the_item in config:
        ...
        the_item = 'entity_dict_dir'
        if the_item in config:
            self.entity_dict_dir = config[the_item]
