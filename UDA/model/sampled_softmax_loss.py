...
class SampledSoftmaxLoss(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 oredered_vocab: bool = False) -> None:
        super().__init__()
        ...
        self.ordered_vocab = oredered_vocab
    ...
    def forward(self,
                embeddings: Variable,
                targets: Variable,
                target_token_embedding: Variable = None) -> torch.Tensor:
    ...
    def _forward_train(self,
        ...
        # [batch_size, ]
        true_logits = (true_w * embeddings).sum(dim=1) + true_b
        if self.ordered_vocab:
            true_logits -= torch.log(target_expected_count + 1e-7)
        # [batch_size, n_samples]
        sampled_logits = (torch.matmul(embeddings, sampled_w.t()) + sampled_b)
        if self.ordered_vocab:
            sampled_logits -= torch.log(sampled_expected_count + 1e-7)
